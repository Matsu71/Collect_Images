...
| 画像を取得できた分類群 | 991 |
| 主画像・補足として残した素材 | **2,364点、984分類群に対応** |
| 姿・主要な特徴が分かる主画像候補を選べた分類群 | **924** |
| 主画像候補の写真×分類群の対応数 | 1093 |
| 補足用の写真×分類群の対応数 | 1272 |
...
**全1,000分類群の主画像が揃ったという意味ではありません。** 花・葉・頭部などの補足写真だけのものや、採用素材がないものもあります。ChronoEarth由来532分類群では、画像取得は523、主画像候補の選定は469です。[主画像未確保リスト](reports/primary-gaps.json) に不足を記録しています。
...
<!-- SESSION-FINAL-20260914 -->
## 第3回の確定結果と微生物写真の補完

第3回の追加163枚に加え、プロクロロコッカスの電子顕微鏡写真1枚を個別に確認して保存しました。今回の新規採用は合計 **164枚**です。既存のプロメテオアルカエウムの画像は再取得・上書きせず、新規件数にも数えていません。

今回の開始時点と比べ、主画像候補は **896 → 924分類群**、採用した写真は **2,200 → 2,364枚**です。ChronoEarth由来の主画像候補は **458 → 469分類群**になりました。残る主画像不足は全体76分類群、ChronoEarth由来63分類群です。
